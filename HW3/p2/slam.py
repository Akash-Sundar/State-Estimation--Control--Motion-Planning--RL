...
class map_t:
    ...
    def grid_cell_from_xy(s, x, y):
        ...
        """
        x_grid = np.ceil((np.clip(x, s.xmin, s.xmax) - s.xmin) / s.resolution).astype(np.int16)  
        y_grid = np.ceil((np.clip(y, s.ymin, s.ymax) - s.ymin) / s.resolution).astype(np.int16) 
...
class slam_t:
    ...
    def get_control(s, t):
        ...
        y = lambda a,b: np.array([a[0], a[1], b[2]])

        return smart_minus_2d(y(s.lidar[t]['xyth'], s.lidar[t]['rpy']), y(s.lidar[t-1]['xyth'], s.lidar[t-1]['rpy']))
